...
```{r project.setup, echo=TRUE, message=FALSE, error=FALSE, warning=FALSE, results='hide'}
#Import libraries
library(tidyverse)
library(here)
library(knitr)
...
```{r Ozone2018, fig.dim=c(6,4), fig.align='left', echo=FALSE, warning=FALSE, fig.cap="Monthly Ozone (2018)"}
epa_data %>%
  filter(year(Date) == 2018) %>%
  mutate(Month = month(Date, label = TRUE, abbr = TRUE)) %>%
  ggplot(aes(x = Month, y = Ozone)) +
  labs(title="Monthly Ozone (2018)", x="Month", y="Ozone (µg/m^3)") +
  geom_boxplot()
```

```{r Ozone2019, fig.dim=c(6,4), fig.align='left', echo=FALSE, warning=FALSE, fig.cap="Monthly Ozone (2019)"}
epa_data %>%
  filter(year(Date) == 2019) %>%
  mutate(Month = month(Date, label = TRUE, abbr = TRUE)) %>%
  ggplot(aes(x = Month, y = Ozone)) +
  labs(title="Monthly Ozone (2019)", x="Month", y="Ozone (µg/m^3)") +
  geom_boxplot()
```
